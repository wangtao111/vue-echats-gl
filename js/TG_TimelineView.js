	...
	*/
	...
	this._templates = {
		...
		+ "</div>"),
     	...
     	filter_modal : $.template( null,
          ...
          "<input placeholder='关键字' type='text' class='timeglider-filter-search'></div>"+
          "<div class='tg-filter-cbs'>"+
          "<input type='checkbox' id='filter_t' checked><label for='filter_t'>标题</label>"+
          "&nbsp;&nbsp;&nbsp;<input type='checkbox' id='filter_d'><label for='filter_d'>摘要</label>"+
          "</div>"+
          ...
          "<ul class='buttons'>"+
          "<li class='timeglider-filter-apply'>确定</li>"+
          "<li class='timeglider-filter-clear'>清除</li>"+
          "</ul></div>"+
        ...
        settings_modal : $.template( null,
          ...
          "<div class='tg-legend-controls'><span class='tg-legend-all'>所有</span><span class='tg-legend-none'> | 清空</span></div>"+
          "</div>"+
          "</div>")
	...
	this.timelineInfoModal = Backbone.View.extend({
		...
		template: function () {
			...
			+ "<ul>" + tags1 + "<li data-timeline_id='" + this.model.get("id") + "' class='tg-timeline-start'>起点</li></ul>" + tags2
	...
	this.presInfoModal = Backbone.View.extend({
		...
		template: function () {
			...
			+ "<ul><li class='tg-close'>close</li><li class='tg-pres-start'>起点</li></ul>"
			+ "<div class='tg-modal-corner tg-modal-corner-north'></div>";
  ...
  registerTitles: function () {
	...
	buildSettingsMenu: function () {
		...
		$s.find(".timeglider-settings-timezone")
			.append('<span class="settings-label">时区:</span> ' + tz_menu + '<br><a class="btn" id="timeglider-settings-save">保存</a>');
	...
	setupSingleTimeline: function() {
		...
		if (MED.options.display_single_timeline_info != false) {
			...
			inf = (timeline.get("description")) ? "<li id='info' class='timeline-info-bt' data-timeline_id='" + tid + "'>摘要</li>":"",
			
			leg = (timeline.get("hasLegend")) ? "<li id='legend' class='tg-legend-bt' data-timeline_id='" + tid + "'>标题</li>":"",
			
			tools = ""; // "<a id='tools' class='tools-bt noselect'>tools</a>",
			
			tmpl = "<div class='tg-widget-header tg-single-timeline-header'>" + title + "<ul>" + inf + leg + "<li class='tg-timeline-start' data-timeline_id='" + tid + "'>回到起点</li></ul>" + tools + "</div>";
	...
	setupPresentation: function() {
		...
		var title = "<h2 id='pres_title' class='no-select' data-timeline_id='primary'>" + pres.title + "</h2>",
			...
			inf = (pres.description) ? "<li id='info' class='pres-info-bt'>摘要</li>":"",
			
			leg = (pres.legend) ? "<li id='legend' data-timeline_id='pres' class='tg-legend-bt'>标题</li>":"",
			
			tools = "", // "<a id='tools' class='tools-bt noselect'>tools</a>",
			
			tmpl = "<div class='tg-widget-header tg-pres-header'>" + title + "<ul>" + inf + leg + "<li class='tg-pres-start'>起点</li></ul>" + tools + "</div>",
